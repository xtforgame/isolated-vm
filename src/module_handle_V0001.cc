...
ModuleInfo::ModuleInfo(Local<Module> handle) : handle(handle) {
	...
	std::cout << "========== ptr : " << IsolateEnvironment::GetCurrent() << std::endl;
    auto& module_map = IsolateEnvironment::GetCurrent()->module_handles;
	for(auto it = module_map.begin(); it != module_map.end(); it++) {
        std::cout << it->first << '\t';
        std::cout << it->second << std::endl;
    }
    std::cout << "handle GetIdentityHash: " << handle->GetIdentityHash() << std::endl;
...
ModuleInfo::~ModuleInfo() {
    ...
    std::cout << "GetIdentityHash: " << handle.Deref()->GetIdentityHash() << std::endl;
